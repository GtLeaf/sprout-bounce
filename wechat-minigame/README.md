...
- 游戏内环境：首页之后继续显示与网页版统一的低多边形浮岛、云朵与瀑布竖屏背景；该资源由公司 AIGC 平台按网页基准图生成，任务 ID `634504129611317669`。
- 首局引导：微信玩家第一次开始挑战时显示与网页版一致的三步实时棋盘引导，可前后切换或直接关闭。
- 完整声音：小游戏包内包含网页版同源的派对循环音乐、跳跃、过关和结算反馈音，并针对手机包体转为低采样率单声道 PCM。
- 微信渲染材质：真机使用不依赖光照着色器的基础低多边形材质并关闭高开销实时阴影，网页继续使用 Standard PBR 材质。
- 手机性能：3D 帧缓冲限制在 1.5 倍像素密度，爆破粒子在微信端按每格 4 个生成，降低高分屏与大连锁时的瞬时压力。
